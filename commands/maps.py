...
from utils.error_embed import map_not_found_embed
...
async def maps(interaction: discord.Interaction, map: app_commands.Choice[str]):
    file_path = os.path.join(MAPS, f"{map.value}.png")
    ...
    try:
        ...
        await interaction.response.send_message(embed=map_not_found_embed(), ephemeral=True)

    except (discord.errors.Forbidden, discord.errors.HTTPException) as e:
        print(f"{Fore.RED}[X] Error sending map: {e}")